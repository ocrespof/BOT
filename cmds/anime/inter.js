...
const pinkieGifs = {
  ...
  sad: [
    ...
    "https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExZWM2MWtiY2o1ZTNyNXI1dnpjZWM1NHJsZ2ZhYng4M3JjdXRxNTJneCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/OUb561ZD45Wxy/giphy.gif",
    "https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExenZkOWIxdnFrdDF4cXFvdnpwMnk1NmMwcmpwbm85dm42YmsxdXB6YyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/Ec7RXFzO4ETO8/giphy.gif",
  ...
  angry: [
    ...
    "https://media1.tenor.com/m/NJw5JqgOd18AAAAd/nopony-breaks-a-pinkie-promise-angry.gif",
...
const symbols = ['(⁠◠⁠‿⁠◕⁠)', '˃͈◡˂͈', '૮(˶ᵔᵕᵔ˶)ა', '(づ｡◕‿‿◕｡)づ', '(✿◡‿◡)', '(꒪⌓꒪)', '(✿✪‿✪｡)', '(*≧ω≦)', '(✧ω◕)', '(⌒‿‿⌒)', '(¬‿¬)', '(✧ω✧)', '✿(◕ ‿◕)✿', 'ʕ•́ᴥ•̀ʔっ', '(ㅇㅅㅇ)', '(∩︵∩)', '(✪ω✪)', '(✯◕‿◕✯)', '(•̀ᴗ•́)و ̑̑']
...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    try {
      let result;
      const category = pinkieGifs[currentCommand] ? currentCommand : (['laugh', 'smile'].includes(currentCommand) ? 'happy' : (['cry', 'pout'].includes(currentCommand) ? 'sad' : null));

      if (category && pinkieGifs[category]) {
        const gifArray = pinkieGifs[category];
        result = gifArray[Math.floor(Math.random() * gifArray.length)];
      } else {
        const query = encodeURIComponent(`pinkie pie ${currentCommand}`);
        const giphyApiKey = '80fK562JEwSX0HM6CjJ6Me2QmOrnIaiB';
        const response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${giphyApiKey}&q=${query}&limit=15&rating=g`);
        const json = await response.json();

        if (json.data && json.data.length > 0) {
          const randomGif = json.data[Math.floor(Math.random() * json.data.length)];
          result = randomGif.images.original.url;
        } else {
          const fallbackArray = pinkieGifs['happy'];
          result = fallbackArray[Math.floor(Math.random() * fallbackArray.length)];
        }
      }

...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    // luckBuff reduces fail threshold (default 0.3 fail chance)
    let failThreshold = 0.3;
    if (user.luckBuff && user.luckBuff.expiresAt > Date.now()) failThreshold -= user.luckBuff.value;
    ...
    if (user.fortuneBuff && user.fortuneBuff.expiresAt > Date.now()) {
      rob = Math.floor(rob * (1 + user.fortuneBuff.value));
    }

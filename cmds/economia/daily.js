...
export default {
  ...
  run: async (client, m, args, usedPrefix) => {
    ...
    if (now < user.lastdaily) {
      // Check if user has extraDaily item
      if (user.extraDaily) {
        user.extraDaily = false; // Consume the item
      } else {
    ...
    if (user.fortuneBuff && user.fortuneBuff.expiresAt > now) recompensa = Math.floor(recompensa * (1 + user.fortuneBuff.value));

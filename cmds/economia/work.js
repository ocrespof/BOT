...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (Date.now() < user.lastwork) {
      return m.reply(`Debes esperar *${formatTime(user.lastwork - Date.now())}* para usar *${usedPrefix + command}* de nuevo.`)
    }
    user.lastwork = Date.now() + cooldown
    let rsl = Math.floor(Math.random() * (4000 - 2000 + 1)) + 2000
    if (user.title === 'title_legend') rsl = Math.floor(rsl * 1.15); // Buff: Leyenda
    if (user.fortuneBuff && user.fortuneBuff.expiresAt > Date.now()) rsl = Math.floor(rsl * (1 + user.fortuneBuff.value));

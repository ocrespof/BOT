...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
      ...
      if (!primaryBotId || primaryBotId === botId) {
        if (respuestaUsuario === respuestaCorrecta) {
          ...
          if (juego.intentos >= 3) {
            ...
            return client.reply(chatId, `「」Respuesta incorrecta, te quedan ${3 - juego.intentos} intentos.`, m)
          }
        }
      }
      return
    ...
    if (["math", "mates"].includes(command)) {
      ...
      global.math[chatId] = {
        juegoActivo: true, problema,
        respuesta: resultado.toString(), intentos: 0,
        dificultad, timeout: Date.now() + 60000,

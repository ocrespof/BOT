  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      for (const [name, pluginModule] of sortedPlugins) {
        ...
        const desc = plugin.desc || plugin.description || '';
        const usage = plugin.usage ? `\n  ↳ _Uso: ${usedPrefix}${primaryCmd} ${plugin.usage}_` : '';
        const cooldown = plugin.cooldown ? ` ⏱️${plugin.cooldown}s` : '';
        
        let entry = `⊳ *$prefix${primaryCmd}*${cooldown}`;
        if (desc) entry += ` - _${desc}_`;
